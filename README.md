...
Data/raw/Motor_Vehicle_Collisions_-_Crashes_20251209.csv
...
4. Run notebooks in this exact order:

   - **Step 1:** `notebooks/clean.ipynb`  
     Cleans raw data and saves `Data/clean/cleaned_crash_data.csv`

   - **Step 2:** `notebooks/eda.ipynb`  
     Explores patterns and engineers features, saves `Data/clean/cleaned_for_modeling.csv`

   - **Step 3:** `models/modelA.ipynb`  
     Builds global mean baseline model

   - **Step 4:** `models/modelB.ipynb`  
     Builds simple logistic regression model

   - **Step 5:** `models/modelC.ipynb`  
     Builds final calibrated logistic regression, saves `app/calibrated_model.pkl`
...
The original group project built a pedestrian injury risk predictor using NYC 
crash data. It included exploratory data analysis, a data cleaning pipeline, 
and a calibrated logistic regression model deployed via a Streamlit app. The 
model correctly flags approximately 84% of pedestrian injury crashes, helping 
NYC Vision Zero identify high-risk conditions proactively.

### What I Individually Expanded or Improved
- **Reproducibility:** Replaced all hardcoded local file paths across every 
  notebook with relative paths so the project runs on any machine without 
  modification
- **Pipeline documentation:** Added opening header cells to every notebook 
  clearly stating its purpose, inputs, outputs, and run order
- **Data structure:** Reorganized the `Data/` folder into `raw/` and `clean/` 
  subfolders to separate raw inputs from processed outputs
- **Notebook clarity:** Removed commented out code and redundant pre-calibration 
  cells from `modelC.ipynb` to make the final workflow clean and portfolio-ready
- **Reproduction Steps:** Rewrote the README reproduction steps with the exact 
  notebook run order and what each step produces

### Why These Changes Improve the Analysis
The original project had strong modeling and analysis but was difficult to 
reproduce — hardcoded paths would break immediately on any machine other than 
the original. These improvements mean anyone can clone the repo and run the 
full pipeline end-to-end without confusion. For a tool meant to support Vision 
Zero decision-making, a reproducible and well-documented pipeline is essential 
for trust and long-term usability.

### New Capabilities Created
- The project can now be run end-to-end by anyone who clones the repository
- The clear notebook headers and run order make the pipeline explainable in a 
  technical interview without needing to open the code
- The reorganized data folder structure follows industry best practices for 
